...
def openvariant(number):
    """
    Прорисовка варианта по номеру number после его выбора в главном меню. Удаляются все прошлые объкты, которые были в массиве massiv_delete. Также запусается таймер.
...
def docheck2(number, vern):
    """
    Показ решений второй части по номеру варианта number, радиокнопки для оценки пользователя своих заданий.
    Также необходимо передать в vern количество набранных баллов в первой части.
    После оценки запускается функция финального экрана по нажатию на кнопку, этот удаляется.
...
def endprogramm(massivradiobutton, vern):
    """
    Финальная функция, удаляются все прошлые объекты из массива massiv_delete, окончательный подсчет
    правильно решенных заданий с помощью набранных баллов в первой части(vern) и подсчета оценки пользователя через
    считывание значений радиокнопок из массива massivradiobutton и вывод информации на экран.
...
def show_variant(write_password):
    ...
    В массиве write_password на 0 позиции передать номер варината, на  1 позиции пароль.
    :param write_password:  введенный пароль и варинат
    """
...
def show_results():
    ...
    которые берет из файла rezults.txt
    """
...
def raschitprocenti(ball):
    """
        Происходит перевод первичных баллов(ball) во вторичные по шкале ЕГЭ
...
def nadpis(procent):
    """
        Функция подбирает авторский комментарий под количество набранных процентов участником(procent)
...
def nadpis_ball(chislo):
    """
        Происходит подбор нужного падежа под количество набранных баллов по числу chislo
